...
export class Stock {
  ...
  async getPrice(wb) {
    const { price } = await wb.ticker(this.ticker);
  ...
  async tick(wb, bank) {
    ...
    await this.getPrice(wb);
    this.change =
      Math.round((this.currentPrice - this.prevPrice) * 1000) / 1000;

    if (this.change >= this.considerBuyPositive) {
      console.log(
        `Ticker ${this.ticker} Prev ${this.prevPrice} Current ${this.currentPrice} Change ${this.change} `,
      );
